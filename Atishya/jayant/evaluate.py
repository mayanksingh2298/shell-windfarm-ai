...
def getAEP(turb_rad, turb_coords, power_curve, wind_inst_freq, 
            n_wind_instances, cos_dir, sin_dir, wind_sped_stacked, C_t, get_each_turbine_power=False, with_deficit = False):
    ...
    if (not get_each_turbine_power) and (not with_deficit):
        return(AEP)
    else:
        return AEP,each_turbine_power,sped_deficit_eff
