...
def save_csv(coords):
	...
	f = open("submissions/local_search_cont.csv",'w')
...
if __name__ == "__main__":
	...
	for i in tqdm(range(999999999999999)):
		total_iterations += 1
		iteration += 1
		...
		for ind, (new_x, new_y) in enumerate(possibilities):
			...
			if new_score >= best_score+IMPROV_THRESH:
		...
		if best_ind == -1:
			...
			save_csv(coords)
			print("new_score:",(old_score,score(coords,wind_inst_freq, False,False,False,False)) ,"Iteration:",total_iterations)	
