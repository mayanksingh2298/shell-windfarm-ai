...
MASTER_IMPROVEMENT_THRESHOLD = 0.000
...
def local_search(wind_inst_freq,child,master_to_child_qlis,child_to_master_qlis, extensive = False):
	while True:
		coords = getTurbLoc(WINNER_CSV)	
		iteration = -1
		total_iterations = 0
...
def local_search_microscope(wind_inst_freq,child,master_to_child_qlis,child_to_master_qlis):
	while True:
		coords = getTurbLoc(WINNER_CSV)	
		iteration = -1
		total_iterations = 0
	...
	for child in range(NPROCS):
		...
		if(pid==0):
			...
			if(child<NEGATIVE_CHILDREN):
				...
				if child%3 == 0:
					local_search(wind_inst_freq,child,master_to_child_qlis,child_to_master_qlis, extensive = False)
				elif child%3 == 1:
					local_search_microscope(wind_inst_freq,child,master_to_child_qlis,child_to_master_qlis)
				else:	
					local_search(wind_inst_freq,child,master_to_child_qlis,child_to_master_qlis, extensive = True)
	#master
	if(pid>0):
