...
if __name__ == "__main__":
	...
	while(True):
		...
		if best_ind == -1:
			...
			good_score = (old_score >= file_score - 0.1)
			super_waste_of_time = iters_with_non_pos_increase >=  50000
			waste_of_time =  iters_with_non_pos_increase >= 10000
			if ((good_score and super_waste_of_time) or (not good_score and waste_of_time)) and entered:
				print("going mad")
				if old_score >= file_score - 0.01:
					save_csv(coords)
